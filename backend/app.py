...
app=FastAPI(title="PC PICK Private",version="5.9-private")
USER=os.getenv("PCPICK_USER","pcpick")
PASSWORD=os.getenv("PCPICK_PASSWORD","")
ELEVENST_KEY=os.getenv("ELEVENST_API_KEY","").strip()
ELEVENST_URL=os.getenv("ELEVENST_API_URL","https://openapi.11st.co.kr/openapi/OpenApiService.tmall").strip()
...
def db():
    ...
    c.execute("""create table if not exists rankings(
      id integer primary key, shop text,title text,price integer,url text,
      product_code text,components text,scores text,captured_at integer,
      unique(shop,url))""")
    return c
...
def validate(raw):
    ...
    if q.scheme not in ("http","https") or not q.hostname or q.username or q.password or port not in (None,80,443):
        raise HTTPException(400,"올바른 상품 URL이 아닙니다.")
    ...
    for x in infos:
        ...
        if ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_reserved or ip.is_unspecified:
            raise HTTPException(400,"안전하지 않은 주소입니다.")
    return raw
...
def parse_specs(text):
    ...
    if not gpu and re.search(r"(AMD\s*)?내장\s*그래픽|integrated\s*graphics",flat,re.I):
        gpu="AMD 내장 그래픽" if re.search(r"AMD|라이젠|Ryzen",flat,re.I) else "내장 그래픽"
...
def score(c,price=None):
    ...
    completeness=round(known/8*100); value=None
...
def save_rank(shop,title,price,url,code,components,scores):
    if not price or not shop or not url:return
    c=db()
    c.execute('insert into rankings(shop,title,price,url,product_code,components,scores,captured_at) values(?,?,?,?,?,?,?,?) on conflict(shop,url) do update set title=excluded.title,price=excluded.price,product_code=excluded.product_code,components=excluded.components,scores=excluded.scores,captured_at=excluded.captured_at',
              (shop,title,price,url,code,json.dumps(components,ensure_ascii=False),json.dumps(scores,ensure_ascii=False),int(time.time())))
    c.commit(); c.close()
...
def parse_page(url,html):
    ...
    title=(e.get("content") or "").strip() if e else ""
    if not title:
    ...
    comps=parse_specs(title+" "+text[:30000]+" "+html[:30000])
    typ="BAREBONE" if "반본체" in (title+" "+text) else ("COMPLETE" if comps.get("GPU") and "내장" not in comps.get("GPU","") else "UNKNOWN")
...
def elevenst_search(keyword,page=1,page_size=50,sort="CP",save=True):
    if not ELEVENST_KEY:raise HTTPException(503,"11번가 API Key가 아직 연결되지 않았습니다.")
    keyword=(keyword or "").strip()
    if not keyword or len(keyword)>100:raise HTTPException(400,"검색어를 확인해 주세요.")
    page=max(1,min(int(page),1000)); page_size=max(1,min(int(page_size),200))
    sort=sort if sort in {"CP","A","G","I","L","H","N"} else "CP"
    params={"key":ELEVENST_KEY,"apiCode":"ProductSearch","keyword":keyword,"pageNum":str(page),"pageSize":str(page_size),"sortCd":sort,"targetSearchPrd":"KOR"}
    try:r=requests.get(ELEVENST_URL,params=params,timeout=20,headers={"User-Agent":"PC-PICK/0.5"})
    except requests.RequestException:raise HTTPException(502,"11번가 Open API 연결에 실패했습니다.")
    if r.status_code>=400:raise HTTPException(502,f"11번가 Open API 응답 오류({r.status_code})")
    try:root=ET.fromstring(r.content)
    except ET.ParseError:raise HTTPException(502,"11번가 Open API XML 응답을 해석하지 못했습니다.")
    if "error" in root.tag.lower():raise HTTPException(502,"11번가 Open API가 오류 응답을 반환했습니다.")
    items=[]
    for p in root.findall(".//Products/Product"):
        code=xtext(p,"ProductCode"); title=xtext(p,"ProductName") or "11번가 상품"
        detail=xtext(p,"DetailPageUrl") or xtext(p,"ProductDetailUrl")
        image=xtext(p,"ProductImage") or xtext(p,"ImageUrl")
        regular=money(xtext(p,"ProductPrice")); sale=money(xtext(p,"SalePrice"))
        candidates=[x for x in (sale,regular) if x and x>=1000]; price=min(candidates) if candidates else None
        comps=parse_specs(title)
        typ="BAREBONE" if "반본체" in title else ("COMPLETE" if any(k in title.upper() for k in ("RTX ","RX ")) else "UNKNOWN")
        sc=score(comps,price)
        item={"shop":"11번가","product_code":code,"title":title,"url":detail,"image_url":image,"seller":xtext(p,"Seller"),"rating":xtext(p,"Rating"),"review_count":money(xtext(p,"ReviewCount")),"prices":{"regular":regular,"sale":sale,"best":price},"components":comps,"type":typ,"scores":sc}
        items.append(item)
        if save and price and detail:save_rank("11번가",title,price,detail,code,comps,sc)
    return {"source":"11st-open-api","keyword":keyword,"page":page,"page_size":page_size,"sort":sort,"count":len(items),"items":items,"saved":bool(save)}
...
@app.get("/api/health")
def health():
    return {"ok":True,"version":"5.9-private","methodology":"2026.09.v1","connectors":{"11st":{"configured":bool(ELEVENST_KEY),"api":"ProductSearch"}}}

@app.get("/api/connectors")
def connectors():
    return {"11st":{"status":"READY" if ELEVENST_KEY else "NEEDS_KEY","apiCode":"ProductSearch","configured":bool(ELEVENST_KEY),"max_page_size":200},"gmarket":{"status":"PARTIAL"},"himart":{"status":"PARTNERSHIP_REQUIRED"},"danawa":{"status":"RESEARCHING_FEED"}}

@app.post("/api/11st/search")
def search_11st(req:ElevenSearch):return elevenst_search(req.keyword,req.page,req.page_size,req.sort,req.save)
...
def analyze_image(req:AnalyzeImageText):
    ...
    comps=parse_specs(text); known=sum(bool(v) for v in comps.values()); status="OK" if known>=6 else "PARTIAL"; sc=score(comps,req.price)
...
def rankings(limit:int=20):
    ...
    out=[]
    for r in rows:out.append({"shop":r["shop"],"title":r["title"],"price":r["price"],"url":r["url"],"product_code":r["product_code"],"components":json.loads(r["components"]),"scores":json.loads(r["scores"]),"captured_at":r["captured_at"]})
